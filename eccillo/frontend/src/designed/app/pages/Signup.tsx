...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const name = form.name.trim().split(/\s+/);
    const username = form.email.trim().split("@")[0].replace(/[^a-zA-Z0-9_.-]/g, "") || "eccillo-user";
    setError("");
    setSubmitting(true);
    try {
      await signIn(await authApi.register({
        username,
        email: form.email,
        password: form.password,
        first_name: name[0] || "Eccillo",
        last_name: name.slice(1).join(" ") || "User",
        organization_name: (form.name.trim() || username) + "'s organization",
